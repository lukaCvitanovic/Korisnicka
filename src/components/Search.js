...
const Listing = (data) => {
    ...
    for(var i = 0; i<data.length; i++) {
        const {articles} = data[i].node
        for(var j = 0; j<articles.length; j++) {
            var {name} = articles[j].node
            list.push(name)
        }
    }
    list = list.sort()
...
class Search extends Component {
    ...
    render() {
        this.list = Listing(this.props.data.edges)
        return(
            <div class="item search">
                <div className="search_box">
                    <input name="search_bar" type="text" placeholder="Pretraži"
...
export default (props) => (
    ...
    query={graphql`
        query search {
            file: allArticlesJson(filter: {name: {ne: "db"}}) {
                edges {
                  node {
                    articles {
                      node {
